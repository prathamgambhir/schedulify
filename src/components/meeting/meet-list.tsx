...
const MeetList: React.FC<meetListProps> = ({ meetings, type }) => {
  if (meetings.length === 0) {
    return (
      <p className="text-lg md:text-xl font-semibold my-5 mx-1">
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-4">
      {meetings.map((meeting) => (
        <Card key={meeting.id} className="my-2 min-h-auto md:min-h-64 w-auto flex flex-col">
          ...
          <CardContent className="flex flex-col gap-3 flex-1 pb-4">
            ...
            {meeting.meetLink && (
              <div className="flex gap-2 items-center text-sm font-medium my-1">
                <Image
